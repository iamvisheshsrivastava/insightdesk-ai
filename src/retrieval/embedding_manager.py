...
Uses fastembed (ONNX-based) instead of sentence-transformers/torch to keep
the process lightweight enough for free-tier hosting (~100MB vs 1GB+ install,
much lower runtime RAM). Produces the same 384-dim all-MiniLM-L6-v2 vectors,
so existing stored embeddings/FAISS indexes remain compatible.
"""
...
try:
    ...
    logging.warning("fastembed not available. Install with: pip install fastembed")
...
class EmbeddingManager:
    ...
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
        ...
        Args:
            model_name: fastembed/HuggingFace model name for sentence embeddings
        """
    ...
    def load_model(self):
        ...
        if not FASTEMBED_AVAILABLE:
            raise RuntimeError("fastembed not available. Install with: pip install fastembed")
    ...
    def encode_texts(self, texts: List[str], batch_size: int = 32) -> np.ndarray:
        ...
        embeddings = np.array(list(self.model.embed(texts, batch_size=batch_size)), dtype="float32")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("💡 Make sure to install fastembed: pip install fastembed")
